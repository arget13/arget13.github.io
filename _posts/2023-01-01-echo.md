...
The reason why the `__free_hook` way is so easy is because when the program calls to `free()` (and through it to the address we place in its hook) it is already passing as argument an address to something we control (the motherfucking chunk to which we just wrote). So if we were to place... I don't know, like `system()` for example, in `__free_hook` and at the beginning of our chunk we wrote `/bin/sh` we may win a shell. Now, we can't place a null terminator for this string, but `;` will do the job just fine.
```python
from pwn import *
