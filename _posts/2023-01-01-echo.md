...
```
Notice this ineffable serendipity, the chunk is still of size 0x120 even though we changed its size to 0x100, this is because this isn't really *our* chunk, this is the fake previous one (0x20) merged with *ours* (now 0x100). The interesting bit is that we now need to allocate that chunk writing only one byte to avoid overwriting the address we want to leak, that would've been impossible if the resulting chunk were of another size, because we already have the stack buffer filled with a string of the size required to allocate a chunk of size 0x120, and it would have been a problem to fix this,<span id="3_"><a href="#3"><sup>3</sup></a> the only way would have been to adjust the fake previous chunk's size to make the resulting chunk of the same size as ours, but by luck we already got that right.<span id="4_"><a href="#4"><sup>4</sup></a> Hmm, I guess explained like that it doesn't seem too impressive, I'm a simple man, I'm easily entertained with simple things, ok? S-Shall we just `r.write(b'A')`?
