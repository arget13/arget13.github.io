...
r = process("./binary")
# We're gonna have to sleep between writes to prevent them from mixing together.
# We could read() the response for each write, but that would mean to determine
# how much will the program print for every request... And since the program
# doesn't prompt actively for data we can't use readuntil().
deltat = .05
...
```
By overwriting the least significant byte of our chunk with a NUL we marked the previous one as free, and thus tries to merge with it. But what is the problem? The previous chunk is free for real, I promise! The problem is that when a tcache chunk (or fastchunk) is freed, `free()` doesn't place its size in the `prev_size` of the next one; it also doesn't clear the `PREV_INUSE` bit in the `size` of the next one —precisely to avoid this exact problem. We could just place the size of the previous chunk in our `prev_size` after overflowing, something like this
```python
### Insert a chunk in unsorted ###
log.info("Allocating 0x20")
r.write(b"A" * (0x20 - 8) + b'\0'); sleep(deltat)
...
AAAAAA00BBBBBB0CCC
```
First we would fill up the stack buffer to the size of the chunk we want to write this in. And now write padding till the last nullbyte, overwriting it, and then the rest of the data.
```
...
```
Using this new toy we can rewrite what we have:
```python
from pwn import *
...
```c
    ...
    if (__builtin_expect (FD->bk != P || BK->fd != P, 0))                     \
        malloc_printerr ("corrupted double-linked list");                     \
```
It checks that the chunk being unlinked is the next to its previous and the previous to its next, which indisputably makes sense. The simplest circularly-linked lists are those with only one sole and lone element (how redundant's that sentence?), which has itself as previous and next (lol I've never thought of me as a circularly-linked list). Notice that we are placing a chunk with size 0x20 inside another chunk, this bigger chunk has 0x10 bytes of headers, so its total size must be 0x30.
```python
...
Now that the backward consolidation has been dealt with we can move *forward* to the forward consolidation. So the partial fake next chunk we built earlier has to be marked as in use, that means that its next chunk has to have the PREV_INUSE bit set. Well, the legitimate next chunk does have the PREV_INUSE bit set, so we can use it. So, at offset 0xf0 we'd place a chunk, which we are going to make the smallest possible too, 0x20, and we can't just place the header because 0x20 byte further we need the legit next chunk. This means that *our* chunk has to be of size 0x120 (0xf0 + 0x20 + our own headers). Putting it together:
```python
### Insert a chunk in unsorted ###
...
create_chunk(0x30, data)
```
Now only remains how to allocate and free the 0x120 chunk without overwriting what we've put in it. We can just reach in the stack buffer the nullbyte we placed there when allocate the 0x30 chunk, this way we will only overwrite the first 0x28 bytes, with the fake chunk safe at offset 0xf0.
```python
...
```
Which also means that contains a couple of pointers to the libc:
```
0x555555604e20: 0x00007ffff7bebca0  0x00007ffff7bebca0
```
Notice this ineffable serendipity, the chunk is still of size 0x120 even though we made it 0x100, this is because this isn't really *our* chunk, this is the fake previous one (0x20) merged with *ours* (0x100). The interesting bit is that we now need to allocate that chunk writing only one byte to avoid overwriting the address we want to leak, that would've been impossible if the resulting chunk were of another size, because we already have the stack buffer filled with a string of the size required to allocate a chunk of size 0x120, and it would be a problem to fix this, the easier way (maybe even the only one) would have been to adjust the fake previous chunk's size to make the resulting chunk of the same size as ours, but by luck we already got it right. Hmm, I guess that explained like that it doesn't seem too impressive, I'm a simple man, I'm easily entertained with simple things, ok? S-Shall we just `r.write(b'A')`?

Huh, nope, I doesn't work, let's see why. If I place a breakpoint at the `malloc()` I can see it is in fact returning *our* chunk but...
```
$rax   : 0x0000555555604e20  →  0x0000000000000000
```
what the *actual* **fuck**? Where the **fuck** is my **fucking** pointer? Why does it **fucking** contain a **fucking** NULL **fucking** pointer? Ok, let me calm down, we will try to understand it later, for now I realized that fortunately it is only the `fwd` pointer, the `bck` pointer is still there, so we only need to step over this the dead body of the first pointer, simple enough.
```python
...
Right now we have to choose where are we going to write and, more importantly, *how*. Don't laugh at me please, but at the moment I lived the realization of the overlapped chunk we just created, by accident really, as something profound, now I know it is the standard technique in this kind of scenarios. Anyway, I think it is kind of beautiful to just reinvent the wheel or rediscover the Mediterranean, I'm certainly having a shitload of fun while learning a lot, I hope the reader isn't putting their hands on their head too much and that they can enjoy my inexperience in the field of heap exploitation.
...
Let's prove we can gain control of rip really easy
```python
log.info("Allocating 0x30 (exploit overlapping chunks)")
data  = b''
data += b"A" * 0x8 # Reach fake chunk's fd pointer within 0x30 chunk
...
The reason why the `__free_hook` way is easy is because when the program calls to `free()` (and through it to the address we place in its hook) it is already passing as argument an address to something we control (the motherfucking chunk to which we just wrote). So if we were to place... I don't know, like `system()` for example, in `__free_hook` and at the beginning of our chunk we wrote `/bin/sh` we may win a shell. Now, we can't place a null terminator for this string, but `;` will do the trick.
```python
from pwn import *
...
* realize that we could escape the tcache by fillin up one of its bins by linking seven chunks in it, which is —I think— the smartest bit of this challenge.
* It was also really important to notice how weird was it that the program would use the string length to allocate but copied only what was just `read()` since (in conjunction with the absence of null terminators placed by the program in the stack) it allowed us to leak, and also gave us a primitive to place nullbytes as we pleased in the chunk, which granted us a surgical precision for crafting fake chunks, which otherwise would have been really difficult if not impossible.
...
Finally I want to thank you too, if someone somehow managed to reach this far I really appreciate it and makes the effort of writing all of this really worth it.

> Ah Bartleby! Ah humanity!


\[1\] Notice that in fact this is preventing *our* chunk from being consolidated with the top chunk (which is the actual next one) and converted into the new top chunk (which would mean it wouldn't go to any bin).  
