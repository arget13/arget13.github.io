...
Fair enough. We place a valid size there and done, right? It's offset 0x100 till the next chunk, but this is from the headers, and we start writing 0x10 bytes beyond the header, so we would need to write at offset `0x100 - 0x10 = 0xf0`, but the field size is 8 bytes further, so 0xf8. Now, what is a valid size? Not too big, not too small. Let's make it the smallest possible, `MINSIZE`, which in 64-bit architectures is defined as 0x20. On the other hand, `free()` also checks that the chunk it is freeing is truly in use by making sure that the next chunk's size's `PREV_INUSE` bit is set, so we are going to place 0x21 as the fake chunk's size.<span id="1"><a href="#1"><sup>1</sup></a>
```python
### Insert a chunk in unsorted ###
log.info("Allocating 0x20")
r.write(b"A" * (0x20 - 8) + b'\0'); sleep(deltat)
...
```
We are making a fake previous chunk with size 0x20 linked against itself. The heap offsets were obtained empirically with gdb.<span id="2"><a href="#2"><sup>2</sup></a>
...
```
Notice this ineffable serendipity, the chunk is still of size 0x120 even though we changed its size to 0x100, this is because this isn't really *our* chunk, this is the fake previous one (0x20) merged with *ours* (now 0x100). The interesting bit is that we now need to allocate that chunk writing only one byte to avoid overwriting the address we want to leak, that would've been impossible if the resulting chunk were of another size, because we already have the stack buffer filled with a string of the size required to allocate a chunk of size 0x120, and it would have been a problem to fix this,<span id="3"><a href="#3"><sup>3</sup></a> the only way would have been to adjust the fake previous chunk's size to make the resulting chunk of the same size as ours, but by luck we already got that right.<span id="4"><a href="#4"><sup>4</sup></a> Hmm, I guess explained like that it doesn't seem too impressive, I'm a simple man, I'm easily entertained with simple things, ok? S-Shall we just `r.write(b'A')`?
