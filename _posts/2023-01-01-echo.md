...
```
Notice this ineffable serendipity, the chunk is still of size 0x120 even though we changed its size to 0x100, this is because this isn't really *our* chunk, this is the fake previous one (0x20) merged with *ours* (0x100). The interesting bit is that we now need to allocate that chunk writing only one byte to avoid overwriting the address we want to leak, that would've been impossible if the resulting chunk were of another size, because we already have the stack buffer filled with a string of the size required to allocate a chunk of size 0x120, and it would have been a problem to fix this\[3\], the only way would have been to adjust the fake previous chunk's size to make the resulting chunk of the same size as ours, but by luck we already got it right.\[4\] Hmm, I guess that explained like that it doesn't seem too impressive, I'm a simple man, I'm easily entertained with simple things, ok? S-Shall we just `r.write(b'A')`?
